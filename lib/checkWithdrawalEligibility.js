		...
		try {
			...
			for (let timelineItem of timelineItems) {
				if (isPullRequest(timelineItem)) {
					...
					tierNumber = tieredCommentRegex(allText);
					console.log(allText);
					console.log('tierNumber in check', tierNumber);
			...
			return resolve({ issueId, canWithdraw: true, type: 'SUCCESS', errorMessage: null, claimantPullRequestUrl: claimantPRFound.url, tier: tierNumber });
		} catch (error) {
			console.error(error);
