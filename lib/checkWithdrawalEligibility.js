...
const checkWithdrawalEligibility = async (issueUrl, token, pat) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			for (let timelineItem of timelineItems) {
				...
				if (isPullRequest(timelineItem)) {
					...
					if (prIsEligible) {
						...
						claimantPrs.push(pullRequest);
						// if bountyType is not competition, break. If it's competition or ongoing, keep going.
						// This is where you would only break if you found an unclaimed, matching PR
						break;
					} else {
			...
			return resolve({ issueId, canWithdraw: true, type: 'SUCCESS', errorMessage: null, claimantAsset: claimantPRFound.url, tier: tierNumber, claimant: viewer });
		} catch (error) {
			console.error(error);
